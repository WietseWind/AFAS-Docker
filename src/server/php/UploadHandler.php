...
class UploadHandler {
    ...
    protected function get_full_url($withToken = false) {
        ...
        return
            ...
            . ($withToken ? '/index.php/token:' . $this->getToken() : '');
    }

    protected function get_user_id() {
        return $this->options['sessionprefix'].session_id();
    }

    protected function get_user_path() {
        if ($this->options['user_dirs']) {
            return (strlen($this->getToken()) > 4 ? $this->getToken().'_' : '') . $this->get_user_id().'/';
        }
        return '';
    }
    ...
    protected function get_scaled_image_file_paths($file_name, $version) {
        ...
        if (!empty($version)) {
            ...
            if (!is_dir($version_dir)) {
                ...
                @file_put_contents($version_dir.DIRECTORY_SEPARATOR.'.htaccess', 'allow from all');
            }
    ...
    protected function get_file_name_param() {
        // WRW fix (no checking existing files)
        if(!isset($_GET['retry']) && $_SERVER["REQUEST_METHOD"] !== "DELETE") return;
